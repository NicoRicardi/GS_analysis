...
raw_en = pd.read_csv("FTMC_en_kcal.csv", index_col=0)
raw_dens = pd.read_csv("FTMC_dens.csv", index_col=0)
...
constant = dict(linestyle="", alpha=0.35, markeredgecolor="k", markeredgewidth=2.5, markersize=10)
...
#for exp in ["ME"]:
    ...
    for n,system in enumerate(systems):
        ...
        s_dens.append(s_den)
        all_ens = s_en["E_MPk"].tolist() + s_en["E_FDET_HF"].tolist() + [s_en["E_ref_HF"][2], s_en["E_ref_HF_CP"][2], s_en["E_ref_MP"][2], s_en["E_ref_MP_CP"][2]]
    ...
    fig_NM, fig_NP, fig_NHF, fig_NMP = [plt.figure(figsize=(20, 10), dpi=150) for i in range(4)]
    axs_NM, axs_NP, axs_NHF, axs_NMP = [[] for i in range(4)]
    for n, system in enumerate(systems):
        ...
        axs_NMP.append(fig_NMP.add_subplot(221+n))#, label="NMP"))
        ...
        axs_NMP[n].set_ylim([emin, emax])
        axs_NMP[n].grid(b=True)
        ...
        axs_MP[n].axhline(y=s_ens[n]["E_ref_MP"][2], color="k", alpha=0.5, linestyle=":")
        axs_MP[n].text(Mmax + 0.01*den_rangeM, s_ens[n]["E_ref_MP"][2], axes_lbls["E_ref_MP"], color="k", ha="left", va="top")
        axs_MP[n].axhline(y=s_ens[n]["E_ref_MP_CP"][2], color="k", alpha=0.5, linestyle="--")
        axs_MP[n].text(Mmax + 0.01*den_rangeM, s_ens[n]["E_ref_MP_CP"][2], axes_lbls["E_ref_MP_CP"], color="k", ha="left", va="bottom")
        axs_NMP[n].axhline(y=s_ens[n]["E_ref_MP"][2], color="k", alpha=0.5, linestyle=":")
        axs_NMP[n].text(Mmax + 0.01*den_rangeM, s_ens[n]["E_ref_MP"][2], axes_lbls["E_ref_MP"], color="k", ha="left", va="top")
        axs_NMP[n].axhline(y=s_ens[n]["E_ref_MP_CP"][2], color="k", alpha=0.5, linestyle="--")
        axs_NMP[n].text(Mmax + 0.01*den_rangeM, s_ens[n]["E_ref_MP_CP"][2], axes_lbls["E_ref_MP_CP"], color="k", ha="left", va="bottom")
        axs_PMP[n].axhline(y=s_ens[n]["E_ref_MP"][2], color="k", alpha=0.5, linestyle=":")
        axs_PMP[n].text(Pmax + 0.01*den_rangeP, s_ens[n]["E_ref_MP"][2], axes_lbls["E_ref_MP"], color="k", ha="left", va="top")
        axs_PMP[n].axhline(y=s_ens[n]["E_ref_MP_CP"][2], color="k", alpha=0.5, linestyle="--")
        axs_PMP[n].text(Pmax + 0.01*den_rangeP, s_ens[n]["E_ref_MP_CP"][2], axes_lbls["E_ref_MP_CP"], color="k", ha="left", va="bottom")
        ...
        axs_PHF[n].plot(s_dens[n]["densdiff_FDET_ref"], s_ens[n]["E_FDET_HF"],
           marker="o", color=colours[0], label="_nolegend_", **constant)
        axs_MP[n].plot(s_dens[n]["M_value"], s_ens[n]["E_MPk"],
           marker="o", color=colours[0], label="_nolegend_", **constant)
        axs_PMP[n].plot(s_dens[n]["densdiff_FDET_ref"], s_ens[n]["E_MPk"],
           marker="o", color=colours[0], label="_nolegend_", **constant)
        ...
        axs_NHF[n].plot(s_dens[n].index, s_ens[n]["E_FDET_HF"],
           marker="o", color=colours[0], label="_nolegend_", **constant)
        axs_NMP[n].plot(s_dens[n].index, s_ens[n]["E_MPk"],
           marker="o", color=colours[0], label="_nolegend_", **constant)
        ...
        axs_NMP[n].set_ylabel(axes_lbls["E_FDET_MP"])
        ...
        axs_NMP[n].set_title(syst_lbls[system])
...
#        axs_NMP[n].set_yscale("symlog")
    ...
    fig_NMP.subplots_adjust(top=0.96, bottom=0.065, left=0.055, right=0.97, wspace=0.18, hspace=0.25)
    ...
    fig_MP.savefig("{}_cycles_M_vs_MP.png".format(exp))
    fig_PMP.savefig("{}_cycles_P_vs_MP.png".format(exp))
    ...
    fig_NMP.savefig("{}_cycles_N_vs_MP.png".format(exp))
