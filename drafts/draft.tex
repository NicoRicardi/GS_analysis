...
To this end, we analyzed the 4 systems from Ref.~\citenum{Ricardi2018}, selected because they display different strength of interaction, number of molecules in the environment, number of non-covalent interactions, charge of the environment, etc...
...
Linearized FDET appears to be notably accurate, giving results which are almost indistinguishable from the fully self-consistent ones, for all systems, including 7Hq formate, where $\Delta^{lin}[\rho_A,\rho_A^{ref},\rho_B^{FT,0}]$ amounts to 19 Kcal/mol. This constitutes the first ground-state extension of the analysis performed for excitations in Ref.~\citenum{Zech2015}.
