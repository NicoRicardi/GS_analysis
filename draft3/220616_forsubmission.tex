...
%$\rho_{AB}(\mathbf{r})-\rho_{B}(\mathbf{r})$ ({\it target density}) {\color{red}is not $N$-representable and} cannot be obtained {\color{red} FDET energy approaches the exact one from above.}
%For an arbitrary choice for $\rho_B$, 
%provides only the upper bound for the total energy. %of the exact energy.
...
\maketitle
%\section*{For Nicco and Cristina}
%{\color{green}
%FOR NICCO AND CRISTINA:
%
%1) note change of the title
%
%2) red - new text
%
%3) blue - to be removed
%
%4)  In the introduction, I used the new notation ( $\rho_{tot(ref)}$ instead of $\rho_{AB}^o(\mathbf{r})$).
% I would like to use it in the remaining text. Any comments?
%
%5) As soon as I get supplementary data from Nicco  and your feedback on  pts 1-4 the revised version is submission ready
%}


\section{Introduction}\label{sec:intro}
The formal framework of 
Frozen-Density Embedding Theory (FDET) provides the basis of multi-scale/multi-level simulation methods that use a multiplicative embedding operator (see the review in Ref. \citenum{Wesolowski2015}).  The self-consistent expressions for the functional of the total energy and the corresponding embedding potential are available for various possible quantum descriptors used for the embedded $N_A$-electron system \cite{Wesolowski1993,Wesolowski2008,Pernal2009,Wesolowski2020}. 
...
%The total energy of a system is given by the functional ${E}_{v_{AB}}^{FDET}[\Psi_{A},\rho_B]$ which is closely related to
...
\begin{itemize} 
\item[{\bf I:}]
the used approximation for the ${E}_{xcT}^{nad}[\rho_A,\rho_B]$ %\approx \tilde{E}_{xcT}^{nad}[\rho_A,\rho_B]$  
component of  ${E}_{v}^{FDET}[\Psi_{A},\rho_B]$ 
%(and the corresponding potential {\color{red} $\frac{\delta\tilde{E}_{xcT}^{nad}[\rho_A,\rho_B]}{\delta\rho(\mathbf{r})}$)}, whereas the other 
and 
\item[{\bf II:}]
the choice for  $\rho_B$.
\end{itemize}
} 
%The approximation used for {\color{red} $\frac{\delta{E}_{xcT}^{nad}[\rho_A,\rho_B]}{\delta\rho(\mathbf{r})}$} leads to the error in the electron density.  
%Since the FDET energy is expressed as a density functional, the error in the energy is the combined effect of the error in 
...
%and the fact that the FDET bi-functional for energy is consistent with the Hohenberg-Kohn functional \cite{Wesolowski2008}, 
%the effect on energy evaluated by means of ${E}_{v}^{FDET}[\Psi_{A},\rho_B]$ 
%due to the choice of  $\rho_B$ is non-negative. 
...
% comprising $N_{tot}$ electrons in the external potential $v_{ext}$. 
...
%to that of the reference calculations
...
%\end{document}
%{\color{red}The necessary condition that  the total density obtained in FDET for a given $\rho_B$ equals 
%to the density $\rho_{tot(ref)}$ obtained from the corresponding supermolecular calculations is this the $N$-representability of the density  $\rho_{tot(ref)}-\rho_B$ that will be refereed to as {\it target density} in the present work.
%This condition is violated is $\rho_{tot(ref)}-\rho_B$ must be negative on some non-zero volume element.
%}
%\end{document}
...
%This work addresses this  issue by means of the numerical procedure allowing to separate the two factors contributing to the errors in the FDET energy: {\it i}) the approximation used for ${E}_{xcT}^{nad}[\rho_A,\rho_B]$ and {\it ii}) the violation of the non-negativity condition for $\rho_{tot(ref)}-\rho_B$. 
%The procedure proposed in this work can be applied for any FDET-based calculations, provided that $\rho_{v}^{o(ref)}$ and the corresponding energy is obtained from the method in which the electron-electron correlation is treated in the same way as in FDET. 
The analysis of the numerical data is made using the second-order M{\o}ller-Plesset results as the reference %numerical examples are given for MP2 calculations  
and the corresponding formal framework based on the extension of FDET for non-variational treatment of electron-electron correlation given in Ref. \citenum{Wesolowski2020}.}

%{\color{blue} The part in blue is to be removed: 
...
%$\int\rho_B(\mathbf{r})d\mathbf{r}=N_B$,
...
\end{eqnarray}
%Similarly to $E_{xct}^{nad}[\rho_A,\rho_B]$, 
%the $\frac{\delta E_{xct}^{nad}[\rho_A,\rho_B]}{\delta\rho_A(\mathbf{r})}\equiv v_{xct}^{nad}[\rho_A,\rho_B](\mathbf{r})$ component of the FDET embedding potential depends on the form of the embedded wavefunction\cite{Wesolowski2008}.
...
%{\color{red}For the sake of completeness, the definitions of the bi-functional $E_{xct}^{nad}[\rho_A,\rho_B]$ as well as other density functionals relevant for the present work are given in the Supplementary Material.}
...
%Only for $v"=v_B$,  $\rho_B^{v"}$ is the Hartree-Fock density in an $N_B$-electron system in the potential $v_B$.
...
\begin{equation} \label{eq:P_bound}
 M[\rho_{B} - \rho^{o(ref)}_{v}] \leq P[\rho^{Eq.\ref{eq:FDET_SE}}_A+\rho_B - \rho_{v}^{o(ref)}] \leq % \int \rho_{AB}^{o} 
 N_{tot}.
\end{equation}
...
%\end{document}
%A more insightful term of comparison can be obtained by comparing the sum of isolated fragments to the reference density by means of $P[\rho_A^{isol}+\rho_B^{isol} - \rho_{AB}^{o}]$. In the interest of compactness, we denote this as
%\begin{equation}\label{eq:p_cmpl}
...
%For consistency, so was the reference density $\rho^{ref}$, used to evaluate $M$ and $P$ parameters.
...
and the parametrisation taken from Ref. \citenum{Vosko1980} of the correlation energy in the uniform electron gas  \cite{Ceperley1980}:
\begin{eqnarray}
...
%In particular, the effect of these additional assumptions on the parameter $M$ will be discussed.
...
As shown in Ref. \citenum{Ricardi2018}, the $\rho_B=\rho_B^{isol}$ approximation in FDET leads to an astonishingly good overall accuracy of the complexation-induced shifts of the vertical valence excitation energies (errors in the order 0.04~eV). 
