...
Any multi-level simulation based based on the formal framework of FDET, %svcomma
...
% The  $E^{c}_{v'_A}$ and  $E_k[\Delta \rho^{c}_{v'_A}, \rho'_A, \rho_B]$
%  terms can be evaluated in practice by means of conventional non-variational methods 
% applied for an auxiliary system of $N_A$ electrons in the potential $v'_A$ given in :    
...
% involves obtaining a self consistent solution of Eq.~\ref{eq:FDET_SE} ($\Phi'_A$) followed by
% solving a "conjoint" equation, in which the indices $A$ and $B$ are switched. 
...
The above iterative procedures, %svcomma
...
The comparative analysis of the three approaches to the electronic polarisation within FDET, %svcomma
...
% the observation that the {\it  monomer expansion} affects much less the errors in the FDET energies  and densities obtained using the {\it supermolecular expansion}
...
% All the above trends are paralleled by the numerical results obtained using the {\it monomer expansion} (see the Supporting Information). 
% {\color{red}TW: to be checked
% 
% The magnitude of the $\Delta^{lin}[\rho_A,\rho_A^{init},\rho_B]$ is systematically smaller than that obtained using the {\it supermolecular expansion} which reflects the fact that additional basis sets centres on atoms in the environment results in larger variation of  $\rho_A(\mathbf{r})$.}
% 
% 
% \begin{figure}[H]
% \centering
% \includegraphics[width=1.0\linewidth]{SE_cycles_N_vs_HF.png}
% \caption{FDET-HF interaction energies for  $\rho_B(\mathbf{r})$ obtained for even-numbered FAT iterations and with the {\it supermolecular expansion} (blue ``X'') and their linearised counterparts ("red +''). 
